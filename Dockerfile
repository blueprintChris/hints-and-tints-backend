...
# 9. docker run -d -p 4000:80 -e SOCKET_KEY=$(aws secretsmanager get-secret-value --secret-id arn:aws:secretsmanager:eu-west-2:978544736059:secret:tints-and-hints-SfrRcn --region eu-west-2 --query SecretString  --output text | cut -d: -f2 | tr -d \"}) 978544736059.dkr.ecr.eu-west-2.amazonaws.com/hints-and-tints-backend
